...
async function createApp() {
    ...
    await database_1.validateDBConnection();
    if ((await database_1.isInstalled()) === false) {
        logger_1.default.error(`Database doesn't have Directus tables installed.`);
        process.exit(1);
    }
    await extensions_2.initializeExtensions();
    await extensions_2.registerExtensionHooks();
    const app = express_1.default();
    const customRouter = express_1.default.Router();
    app.disable('x-powered-by');
    app.set('trust proxy', true);
    app.set('query parser', (str) => qs_1.default.parse(str, { depth: 10 }));
    await emitter_1.emitAsyncSafe('init.before', { app });
    await emitter_1.emitAsyncSafe('middlewares.init.before', { app });
    ...
    await emitter_1.emitAsyncSafe('middlewares.init.after', { app });
    await emitter_1.emitAsyncSafe('routes.init.before', { app });
    ...
    await emitter_1.emitAsyncSafe('routes.custom.init.before', { app });
    await extensions_2.registerExtensionEndpoints(customRouter);
    await emitter_1.emitAsyncSafe('routes.custom.init.after', { app });
    app.use(not_found_1.default);
    app.use(error_handler_1.default);
    await emitter_1.emitAsyncSafe('routes.init.after', { app });
    // Register all webhooks
    await webhooks_2.register();
