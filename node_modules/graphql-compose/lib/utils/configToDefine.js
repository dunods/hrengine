...
const invariant_1 = __importDefault(require("graphql/jsutils/invariant"));
...
function defineFieldMap(config, fieldMap, parentAstNode) {
    var _a, _b, _c;
    invariant_1.default(isPlainObj(fieldMap), `${config.name} fields must be an object with field names as keys or a ` +
    ...
    for (const fieldName of Object.keys(fieldMap)) {
        ...
        invariant_1.default(isPlainObj(fieldConfig), `${config.name}.${fieldName} field config must be an object`);
        ...
        invariant_1.default(field.resolve == null || typeof field.resolve === 'function', `${config.name}.${fieldName} field resolver must be a function if ` +
        ...
        if (!argsConfig) {
            ...
            invariant_1.default(isPlainObj(argsConfig), `${config.name}.${fieldName} args must be an object with argument names as keys.`);
    ...
    Object.keys(_fields).forEach((n) => {
        ...
        if (Array.isArray(fc.args)) {
            fc.args.forEach((arg) => {
                ...
                if ((_a = ac === null || ac === void 0 ? void 0 : ac.astNode) === null || _a === void 0 ? void 0 : _a.directives) {
                    const directives = sc.typeMapper.parseDirectives(ac.astNode.directives);
                    if (directives) {
                        if (!args[argName].extensions)
        ...
        if ((_a = fc === null || fc === void 0 ? void 0 : fc.astNode) === null || _a === void 0 ? void 0 : _a.directives) {
            const directives = sc.typeMapper.parseDirectives(fc.astNode.directives);
            if (directives) {
                if (!fields[n].extensions)
                    fields[n].extensions = {};
...
function defineEnumValues(type, valueMap, parentAstNode) {
    var _a;
    invariant_1.default(isPlainObj(valueMap), `${type.name} values must be an object with value names as keys.`);
    ...
    return Object.keys(valueMap).map((valueName) => {
        ...
        invariant_1.default(isPlainObj(value), `${type.name}.${valueName} must refer to an object with a "value" key ` +
            `representing an internal value but got: ${misc_1.inspect(value)}.`);
        invariant_1.default(!value.hasOwnProperty('isDeprecated'), `${type.name}.${valueName} should provide "deprecationReason" instead of "isDeprecated".`);
        return {
            name: valueName,
...
function defineInputFieldMap(config, fieldMap, parentAstNode) {
    ...
    for (const fieldName of Object.keys(fieldMap)) {
        ...
        invariant_1.default(!field.hasOwnProperty('resolve'), `${config.name}.${fieldName} field has a resolve property, but ` +
