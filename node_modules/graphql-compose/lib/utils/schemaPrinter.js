...
function printFilteredSchema(schema, directiveFilter, typeFilter, options) {
    ...
    const types = objectValues_1.default(typeMap)
...
function printType(type, options) {
    ...
    invariant_1.default(false, `Unexpected type: ${inspect_1.default(type)}`);
    return '';
}
...
function printSpecifiedByUrl(type, options) {
    if (!type.specifiedByUrl || (options === null || options === void 0 ? void 0 : options.omitSpecifiedByUrl)) {
        return '';
    }
    const url = type.specifiedByUrl;
    const urlAST = astFromValue_1.astFromValue(url, scalars_1.GraphQLString);
    if (!urlAST)
        return '';
    return ` @specifiedBy(url: ${printer_1.print(urlAST)})`;
}
...
function printInputObject(type, options) {
    let fields = objectValues_1.default(type.getFields());
...
function printDeprecated(fieldOrEnumVal) {
    ...
    const reasonAST = astFromValue_1.astFromValue(reason, scalars_1.GraphQLString);
    if (reasonAST && reason !== directives_1.DEFAULT_DEPRECATION_REASON) {
        return ` @deprecated(reason: ${printer_1.print(reasonAST)})`;
    }
    return ' @deprecated';
}
...
function printDescription(def, options, indentation = '', firstInBlock = true) {
    ...
    const blockString = blockString_1.printBlockString(description, '', preferMultipleLines);
